  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="出生日期" align="center" min-width="145">
        <template slot-scope="scope">
          {{ scope.row.birthday | dateFormatter}}
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getMyISAMList(this.listQuery).then(response => {
      ...
      createData(){
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            insertMyISAM(this.temp).then((response) => {
              this.getList();
      ...
      handleDelete(index, row) {
        deleteConfirm(() => {
          deleteByIdMyISAM(row.id).then(response => {
            successNotify('删除成功');
